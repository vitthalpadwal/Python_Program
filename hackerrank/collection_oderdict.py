...
for i in range(int(input())):
    record = re.split(r'(\d+)$', input().strip())
    if record[0] in ordered_dic:
        ordered_dic[record[0]] = ordered_dic[record[0]] + int(record[1])
    else:
        ordered_dic[record[0]] = int(record[1])

for name in ordered_dic:
    print(name+str(ordered_dic[name]))