"""
he itertools module standardizes a core set of fast, memory efficient tools that are useful by themselves or in combination. Together, they form an iterator algebra making it possible to construct specialized tools succinctly and efficiently in pure Python.
...
letters = input().split()
k = int(input())
combi_list = list(combinations(letters,k))
a_list = [i for i in combi_list if 'a' in i]
print(len(a_list) / len(combi_list))
